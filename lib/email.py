...
def generate_email_templates(metadata):

    renamed_text = ""
    if metadata["renamed"] == "True":
    ...
    with open(app.email_template_file) as f:
        template = f.read()

    teams_support_snippet = generate_teams_support_snippet(metadata)
    contact_support_snippet = generate_contact_support_snippet(metadata)

    email_html = (
        template.replace("%teams_support_snippet%", f"{teams_support_snippet}")
        ...
        .replace("%hostname%", f"{metadata['hostname']}")
...
def generate_teams_support_snippet(metadata):

    with open(app.snippet_teams_support_filename) as f:
        template = f.read()
...
def generate_contact_support_snippet(metadata):
...
def send_email(who: str, generated_template: dict):
    ...
    for receiver in receiver_emails:
        ...
        with smtplib.SMTP(f"{app.server}", f"{app.server_port}") as server:
